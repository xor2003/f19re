    ...
    while True:
        ...
        if segment in config.code_segments:
            ...
            # some instructions outside a procedure
                ...
                # Emit lines outside procedures verbatim (orphan code, detached
                # function chunks, labels, assumes, publics, alignment). Only
                # --noproc eliminates them.
                if noProc:
        ...
        if not skipWrite:
            ...
            # generic substitutions from config (skipped when a config
            # replace/insert already handled this line)
            if not actionMatch and config.subs:
                for pat, rep in config.subs:
                    writeList = [ pat.sub(rep, w) for w in writeList ]
                debug(f"Applied substitutions: {writeList}")

            # more complex substitutions (skipped when a config
            # replace/insert already handled this line)
            # TODO: make more generic, configurable from conffile
            if not actionMatch and (match := Regex.ALIGN.match(instr)) is not None:
                align = int(match.group(1), 16)
                count = align - (offset % align) 
                comment += instr
                if segment in config.code_segments:
                    # MASM-compatible assemblers pad code alignment with nop
                    writeList = [ 'nop' ] * count
            ...
            elif not actionMatch and (match := Regex.INSTR_AX.match(instr)) is not None:
                comment += instr
                opcode = opcode_map[match.group(1)]
                op2 = match.group(2)
                writeList = [ f'db {opcode}', f'dw {op2}' ]
            elif not actionMatch and (instr == 'jmp far ptr 0:0' or Regex.JMPSLOT.match(instr)):
                comment += instr
                writeList = [ 'db 0EAh', 'dd 0' ]
            elif not actionMatch and instr == 'repne movsw':
                comment += instr
                writeList = [ 'dw 0A5F2h' ]
            elif not actionMatch and instr == 'or di, 2':
                comment += instr
                writeList = [ 'dd 2CF81h' ]
